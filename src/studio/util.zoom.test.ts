...
import { beatZoomStyle, beatFrameFilters, isBeatZoomActive } from "./util";

const zoomChain = (zoom?: number, zoomX?: number, zoomY?: number) =>
  beatFrameFilters(1920, 1080, { zoom, zoomX, zoomY }).base;
...
describe("beat zoom", () => {
  it("is a no-op at 1x", () => {
    ...
    expect(zoomChain(1, 0, 0)).toEqual([]);
    expect(zoomChain(undefined)).toEqual([]);
  });

  it("scales+crops centered by default (ffmpeg)", () => {
    expect(zoomChain(2, 0, 0)).toEqual([
      "scale=3840:2160",
  ...
  it("pans the crop to the edges via focus (ffmpeg)", () => {
    ...
    expect(zoomChain(2, -50, 0)[1]).toBe("crop=1920:1080:0:540");
    // focusX = 1 → right edge, cropX = margin = 1920
    expect(zoomChain(2, 50, 0)[1]).toBe("crop=1920:1080:1920:540");
    // focusY = 1 → bottom, cropY = margin = 1080
    expect(zoomChain(2, 0, 50)[1]).toBe("crop=1920:1080:960:1080");
  });
